...
public class MainVerticle extends AbstractVerticle {
	...
	private void getPistas() {
		mySqlClient.query("SELECT * FROM dad_database.pistas;", res -> {
			if (res.succeeded()) {
				// Get the result set
	...
	private void getPistasWithConnection() {
		mySqlClient.getConnection(connection -> {
			if (connection.succeeded()) {
				connection.result().query("SELECT * FROM dad_database.pistas;", res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
						System.out.println(resultSet.size());
	...
	private void getPorNombrePista(String nombre) {
		mySqlClient.getConnection(connection -> {
			if (connection.succeeded()) {
				connection.result().preparedQuery("SELECT * FROM dad_database.pistas WHERE nombre = ?",
						Tuple.of(nombre), res -> {
							if (res.succeeded()) {
								// Get the result set
								RowSet<Row> resultSet = res.result();
